...
library(tidyverse)
library(reshape)
#library(randomForest) # issue Error in library(gam) : \u2018gam\u2019 is not a valid installed package
...
# (1-1) FNN Function with spatical correction
FNN_sp_V0 <- function(resp, 
                      func_cov, 
                      scalar_cov = NULL,
                      basis_choice, 
                      spatial_basis, # added: N times H
                      num_basis,
                      hidden_layers,
                      neurons_per_layer,
  ...
  if(is.null(scalar_cov)){
    converted_df <- data.frame(matrix(nrow = dim(func_cov)[2],
                                      ncol = (sum(num_basis)*ncol(spatial_basis) )))
  } else {
    converted_df <- data.frame(matrix(nrow = dim(func_cov)[2],
                                      ncol = sum(num_basis)*ncol(spatial_basis) + ncol(scalar_cov)*ncol(spatial_basis) ))
  }
  ...
  for (i in 1:dim(func_cov)[3]) {
    ...
    if(i == 1){
      left_end = 1
      right_end = cur_basis_num*ncol(spatial_basis)
    } else {
      left_end = sum(num_basis[1:(i - 1)])*ncol(spatial_basis) + 1 ## need to double-check 
      right_end = (left_end - 1) + cur_basis_num*ncol(spatial_basis)
    }
    
    if(cur_basis == "fourier"){   # modified
      for (j in 1:ncol(df)) {
        converted_df[j, left_end:right_end] <- c(integral_form_fourier(rep(df[,j], each=ncol(spatial_basis))*rep(spatial_basis[j,], length(df[,j])), 
                                                                       num_beta_basis = cur_basis_num*ncol(spatial_basis),
                                                                       range = cur_range))
      }
    } else{
  ...
  if(is.null(scalar_cov)){
    ...
    for (k in 1:nrow(converted_df)) {
      converted_df[k, (sum(num_basis)*ncol(spatial_basis) + 1):(sum(num_basis)*ncol(spatial_basis) + ncol(scalar_cov)*ncol(spatial_basis))] <- rep(scalar_cov[k,], each=ncol(spatial_basis))*rep(spatial_basis[k,], length(scalar_cov[k,]))
    }
  }
...
# (1-2) FNN Function with spatical correction
# modified 7/19/21
FNN_sp <- function(resp, 
                   func_cov, 
                   scalar_cov = NULL,
                   basis_choice, 
                   spatial_basis, # added: N times H
  ...
  integral_form_fourier <- function(functional_data, 
    ...
    # Setting up vectors
    # 07/19/23 modified without intercept
    fd_basis_form<-c()
    for (i in 1:((num_fd_basis )/2)) {
      fd_basis_form<- c(fd_basis_form, 
                        c(paste0("sin(2*pi*x*", i, "/", range[2], ")"), 
                          paste0("cos(2*pi*x*", i, "/", range[2], ")")))
    }
    
    # Putting together
    #fd_basis_form <- c(1, rbind(func_basis_sin, func_basis_cos))
    #fd_basis_form <- rbind(func_basis_sin, func_basis_cos)
    ...
    beta_basis_form <- c()
    for (i in 1:((num_fd_basis )/2)) {
      beta_basis_form<- c(beta_basis_form, 
                          c(paste0("sin(2*pi*x*", i, "/", range[2], ")"), 
                            paste0("cos(2*pi*x*", i, "/", range[2], ")")))
    }
  ...
  if(is.null(scalar_cov)){
    converted_df <- data.frame(matrix(nrow = dim(func_cov)[2],
                                      ncol = (sum(num_basis)*ncol(spatial_basis) + ncol(spatial_basis))))
  } else {
    converted_df <- data.frame(matrix(nrow = dim(func_cov)[2],
                                      ncol = sum(num_basis)*ncol(spatial_basis) + ncol(scalar_cov)*ncol(spatial_basis) + ncol(spatial_basis)))
  }
  ...
  for (i in 1:dim(func_cov)[3]) {
    ...
    if(cur_basis == "fourier"){   # modified
      for (j in 1:ncol(df)) {
        tmp <- c(integral_form_fourier(df[,j], num_beta_basis = cur_basis_num,range = cur_range))
        converted_df[j, left_end:right_end] <- rep(tmp,each=ncol(spatial_basis)) * rep(spatial_basis[j,], length(tmp))
      }
    } else{
  ...
  # Now attaching spatial basis
  for (l in 1:nrow(converted_df)) {
    converted_df[l,(ncol(converted_df)-ncol(spatial_basis)+1):ncol(converted_df)  ] <-spatial_basis[l,]
  }
...
# (1-3) FNN Function with spatical correction + spatial correction separate
FNN_sp_V2 <- function(resp, 
                      func_cov, 
                      scalar_cov = NULL,
                      basis_choice, 
                      spatial_basis, # added: N times H
  ...
  if(is.null(scalar_cov)){
    converted_df <- data.frame(matrix(nrow = dim(func_cov)[2],
                                      ncol = (sum(num_basis)*ncol(spatial_basis) +ncol(spatial_basis_cov))))
  } else {
    converted_df <- data.frame(matrix(nrow = dim(func_cov)[2],
                                      ncol = sum(num_basis)*ncol(spatial_basis) + ncol(scalar_cov)*ncol(spatial_basis) +ncol(spatial_basis_cov)))
  }
  ...
  for (i in 1:dim(func_cov)[3]) {
    ...
    if(cur_basis == "fourier"){   # modified
      for (j in 1:ncol(df)) {
        tmp <- c(integral_form_fourier(functional_data=df[,j], num_beta_basis = cur_basis_num, range = cur_range))
        converted_df[j, left_end:right_end] <- rep(tmp,each=ncol(spatial_basis)) * rep(spatial_basis[j,], length(tmp))
      }
    }
  }
  ...
  # Now attaching spatial basis
  for (l in 1:nrow(converted_df)) {
    converted_df[l,(ncol(converted_df)-ncol(spatial_basis_cov)+1):ncol(converted_df)  ] <-spatial_basis_cov[l,]
  }
...
############################################################
# for data application - beta allows intercept coefficient
############################################################
FNN_sp_V2_dat <- function(resp, 
                          func_cov, 
                          scalar_cov = NULL,
                          basis_choice, 
                          spatial_basis, # added: N times H
  ...
  }else if(is.na(spatial_basis_cov[1])){
    if(is.null(scalar_cov)){
      converted_df <- data.frame(matrix(nrow = dim(func_cov)[2],
                                        ncol = (sum(num_basis)*ncol(spatial_basis))))
    } else {
...
FNN_Predict_sp_V0 = function(model,
                             func_cov, 
                             scalar_cov = NULL,
                             spatial_basis,
                             basis_choice, 
...
FNN_Predict_sp_V2 = function(model,
                             func_cov, 
                             scalar_cov = NULL,
                             spatial_basis,
                             spatial_basis_cov,
...
FNN_Predict_sp_V2_dat = function(model,
                                 func_cov, 
                                 scalar_cov = NULL,
                                 spatial_basis,
                                 spatial_basis_cov,
...
############################################################
# for data application - spatial varying weights but no kriging
############################################################
FNN_sp_V3_dat <- function(resp, 
                          func_cov, 
                          scalar_cov = NULL,
                          basis_choice, 
                          spatial_basis, # added: N times H
...
FNN_Predict_sp_V3_dat = function(model,
                                 func_cov, 
                                 scalar_cov = NULL,
                                 spatial_basis,
                                 #spatial_basis_cov,
